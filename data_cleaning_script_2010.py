...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  5 05:58:54 2020
...
def add_totals(school_codes, grade):
    ...
    for code in school_codes:
        total_students = df_grades.loc[(df_grades['SCHOOL CODE'] == code) & (df_grades['GRADE'] == grade), 'TOTAL REGISTER'].sum()
        df_grades.loc[(df_grades['SCHOOL CODE'] == code) & (df_grades['GRADE'] == grade), 'TOTAL NUMBER OF STUDENTS'] = total_students
        num_sections = df_grades.loc[(df_grades['SCHOOL CODE'] == code) & (df_grades['GRADE'] == grade), 'NUMBER OF CLASSES'].sum()
...
def add_math_scores(school_DBNs, grade):
    ...
    for dbn in school_DBNs:
        math_score = raw_math_df.loc[(raw_math_df['DBN'] == dbn) & (raw_math_df['Grade'] == grade) & (raw_math_df['Year'] == 2010), 'Mean Scale Score'].sum()
...
def add_ela_scores(school_DBNs, grade):
    ...
    """
    for dbn in school_DBNs:
        ela_score = raw_ela_df.loc[(raw_ela_df['DBN'] == dbn) & (raw_ela_df['Grade'] == grade) & (raw_ela_df['Year'] == 2010), 'Mean Scale Score'].sum()
...
def add_frl_percents(school_DBNs):
    ...
    """
    for dbn in school_DBNs:
        frl_percent = raw_dem_df.loc[(raw_dem_df['DBN'] == dbn) & (raw_dem_df['schoolyear'] == 20092010), 'frl_percent'].sum()
...
if __name__ == '__main__':
    ...
    raw_df = pd.read_csv("Class Sizes/2009_-_2010_Class_Size_-_School-level_Detail.csv")
    ...
    raw_sam_df = pd.read_csv("Controls & Demographics/sam10.csv")
    ...
    # list of all the school DBN codes for schools that have grades 3-8
    school_DBN_sam_list = raw_sam_df['DBN'].unique()[:-1]
    ...
    df_grades['2011'] = pd.Series(0, index=df_grades.index)
    
    
    # export data as CSV
    df_grades.to_csv("full_data_2010.csv", sep=',', index=False)